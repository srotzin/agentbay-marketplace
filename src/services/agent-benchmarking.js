...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS agent_benchmark_comparisons (
    id          TEXT PRIMARY KEY,
...
export async function compareAgents(args) {
  ...
  const id = uuid();
  db.prepare(`
    INSERT INTO agent_benchmark_comparisons (id, agent_a_id, agent_b_id, task_type, winner, margin_pct, test_count)
    VALUES (?, ?, ?, ?, ?, ?, ?)
...
export function getAgentBenchmarkProfile(args) {
  ...
  const comparisons = db.prepare(`
    SELECT * FROM agent_benchmark_comparisons
